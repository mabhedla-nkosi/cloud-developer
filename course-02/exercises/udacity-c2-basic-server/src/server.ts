...
(async () => {
  ...
  app.get("/cars/",(req:Request, res:Response)=>{
    let {make}=req.params;
    // if(!make){
    //   return res.status(404).send("make is required");
    // }
    let cars_list=cars;
    if(make){
  ...
  app.post("/cars",(req:Request, res:Response)=>{
    ...
    cars.push(newCar);
    return res.status(201).send({message: "Your record has been saved!", data: newCar});
    //return res.status(201).send("Your record has been saved!\n"+JSON.stringify(newCar));
